...
use crate::network::behaviour::KadResult;
use cid::Cid;
use core::pin::Pin;
...
impl From<PeerId> for SubscriptionKind {
    fn from(peer: PeerId) -> Self {
        Self::Connect(peer)
    }
}

impl From<Cid> for SubscriptionKind {
    fn from(cid: Cid) -> Self {
        Self::GetBlock(cid)
    }
}

impl From<QueryId> for SubscriptionKind {
...
/// Events used to communicate to the swarm on storage changes.
#[derive(Debug)]
pub enum SubscriptionEvent {
    ...
    NewBlock(
        Cid,
        oneshot::Sender<Result<SubscriptionFuture<KadResult, String>, Error>>,
    ),
    /// Signals the removal of a block.
    RemovedBlock(Cid),
}

impl TryFrom<SubscriptionKind> for SubscriptionEvent {
    type Error = &'static str;

    fn try_from(req: SubscriptionKind) -> Result<Self, Self::Error> {
        if let SubscriptionKind::GetBlock(cid) = req {
            Ok(SubscriptionEvent::UnwantBlock(cid))
        } else {
...
pub type Subscriptions<T, E> =
    HashMap<SubscriptionKind, HashMap<SubscriptionId, Subscription<T, E>>>;
...
impl<T: Debug + Clone + PartialEq, E: Debug + Clone> SubscriptionRegistry<T, E> {
    ...
    pub fn create_subscription(
        ...
        cancel_notifier: Option<Sender<SubscriptionEvent>>,
    ...
    ///
    pub fn finish_subscription(&self, req_kind: SubscriptionKind, result: Result<T, E>) {
...
impl<T, E> Subscription<T, E> {
    fn new(cancel_notifier: Option<Sender<SubscriptionEvent>>) -> Self {
        Self::Pending {
    ...
    fn cancel(&mut self, id: SubscriptionId, kind: SubscriptionKind, is_last: bool) {
        ...
        } = former_self
        {
            ...
            if is_last {
                if let Some(mut sender) = cancel_notifier {
                    ...
                    let _ = sender.try_send(SubscriptionEvent::try_from(kind).unwrap());
                }
            }
